...
Assets here were reused directly from the `/interior-detailing` landing page's shared
brand/hero photography where they already fit this page's two services.

## /images/hero/
| File | Recommended size | Notes |
|---|---|---|
| `hero-bg.jpg` | 1920×1080 (16:9), < 250KB | Reused from the main hero carousel. This is the LCP image; re-compress if you swap it. |
| `og-cover.jpg` | 1200×630 | Still needed. Used for Open Graph / Twitter share previews (`index.html` `<meta property="og:image">`). |

## /images/gallery-images/
Two real before/after photos, one per service, already composited into a single
...
Both are 1536×1024 (3:2). The gallery section in `index.html` (`.gallery__grid.gallery__grid--duo`)
